...
def add_question(cur,userid, dataset, dataitem, detail='',unanswered=1): #set unanswered=0 if we don't need a reply
    ...
    data = cur.fetchone();
  #  print "COUNT %s %s %s" % (dataset, dataitem, detail)
  #  print data[0]
    if (data[0]==0):
...
def set_answer_to_new_question(cur,userid, dataset, dataitem, detail, answer):
    print 'Content-Type: text/html\n'
    print '<html><body>'
#   print "setting %s %s %s %s %s" % (userid,dataset,dataitem,detail,answer)
